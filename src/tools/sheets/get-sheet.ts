...
export const getSheet = defineTool({
  name: "get_sheet",
  description: {
    summary: "根据工作表 ID 查询工作表属性信息，包括工作表的标题、索引位置、是否被隐藏等",
    bestFor: "查询指定工作表的详细属性，如标题、索引、是否隐藏、行列数、冻结行列、合并单元格信息等",
    notRecommendedFor: "获取电子表格中所有工作表列表请使用 query_sheets，获取电子表格基础信息请使用 get_spreadsheet",
  },
  inputSchema: {
    ...
    sheet_id: z.string().describe("工作表的 ID，可调用 query_sheets 获取"),
  },
  outputSchema: {
    sheet: sheetSchema.optional().describe("工作表属性信息"),
  },
  callback: async (context, args) => {
    ...
    try {
      ...
      const response = await context.client.sheets.v3.spreadsheetSheet.get(
        {
          path: {
